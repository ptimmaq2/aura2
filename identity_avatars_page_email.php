<?php
    ...
    $ChangeEmail["oldEmail"] = getInformationUSR($_SESSION['remember_me_email'], "email");
    $ChangeEmail["password"] = getInformationUSR($_SESSION['remember_me_email'], "password");
    $ChangeEmail["birthDay"] = getInformationUSR($_SESSION['remember_me_email'], "birthday_1");
    $ChangeEmail["birthMonth"] = getInformationUSR($_SESSION['remember_me_email'], "birthday_2");
    $ChangeEmail["birthYear"] = getInformationUSR($_SESSION['remember_me_email'], "birthday_3");
    ...
    if(isset($_POST["changeEmail"])) {
        ...
        $TheOldEmail = $_POST["oldemail"];
        $TheNewEmail = $_POST["newemail"];
        ...
        if($TheOldEmail !== $ChangeEmail["oldEmail"]) {
            $error = $LANG["identity"]["emailchange"]["error_1"];
            $errorSent = true;
        }
        
        if($ThePassword !== $ChangeEmail["password"]) {
            if($errorSent == true) {
                $error.= '<br />';
            }
            $error .= $LANG["identity"]["emailchange"]["error_2"];
            $errorSent = true;
        }
        
        if($TheBirth1 !== $ChangeEmail["birthDay"] || $TheBirth2 !== $ChangeEmail["birthMonth"] || $TheBirth3 !== $ChangeEmail["birthYear"]) {
            if($errorSent == true) {
                $error.= '<br />';
            }
            $error .= $LANG["identity"]["emailchange"]["error_3"];
            $errorSent = true;
        }
        
        if($errorSent == false) {
            if($TheNewEmail == $ChangeEmail["oldEmail"]) {
                $errorSent = true;
                $error = $LANG["identity"]["emailchange"]["error_4"];
            }else{
                // Check if this email adres exists in the database (for protecting accounts hack)
                
                $TheNewEmail = sql_stripslashes($TheNewEmail);
                
                $checkMyEmail = $conn->query("SELECT * FROM users_spec WHERE email = '". $TheNewEmail ."'");
                
                if($checkMyEmail->num_rows == 1) {
                    $errorSent = true;
                    $error = $LANG["identity"]["emailchange"]["error_5"];
                }else{
                    // Not used for a other account
                    
                    $updateTheEmail = $conn->query("UPDATE users_spec SET email = '". $TheNewEmail ."'
...
?><html>
<head>
    <title><?php echo $_WEBSITE["HTL"]["name"]; ?> Hotel: <?php echo $LANG["identity"]["emailchange"]["title"]; ?></title>
...
<body>
    <div id="container">
        <div class="settings-container clearfix">
            <h1><?php echo $LANG["identity"]["emailchange"]["emailchangetitle"]; ?></h1>
            <div id="back-link"><img src="<?php echo $_WEBSITE["CMS"]["url"]; ?>/images/identity/hotel.png" id="back-to-main" onclick="document.location.href='<?php echo $_WEBSITE["CMS"]["url"]; ?>'" />
                <a href="<?php echo $_WEBSITE["CMS"]["url"]; ?>/identity/avatars"><?php echo $LANG["identity"]["settings"]["mynames"]; ?></a> » <a href="settings"><?php echo $LANG["identity"]["settings"]["email_login_toptitle"]; ?></a> » <?php echo $LANG["global"]["email"]; ?> 
            </div>
            
            <div style="padding: 0 10px">
                <p><b><?php echo $LANG["identity"]["emailchange"]["title_goodtochange"]; ?></b></p>
                <p><?php echo $LANG["identity"]["emailchange"]["text_goodtochange"]; ?></p>
                ...
                <?php
                    if(isset($successs) && $successs == true) {
                        ?><div class="error_green"><b><?php echo $LANG["global"]["done"]; ?>!</b><br />
                            <?php echo $LANG["identity"]["emailchange"]["succesfull"]; ?> <i><?php
                                echo $TheNewEmail;
                            ?></i>
                        </div><br /><?php
                    }else{
                ...
                <form method="POST" autocomplete="off">
                    <p><b><?php echo $LANG["identity"]["emailchange"]["change_email"]; ?></b></p>
                    <p><?php echo $LANG["identity"]["emailchange"]["change_email_old"]; ?>:</p>
                    <input type="text" placeholder="<?php echo $LANG["identity"]["emailchange"]["change_email_old"]; ?>" class="textOrpasswordT" name="oldemail"<?php
                    if(isset($_POST["oldemail"])) {
                        	   echo ' value="'. $_POST["oldemail"] .'"';
                        	} ?> />
                    <p><?php echo $LANG["identity"]["emailchange"]["change_email_new"]; ?>:</p>
                    <input type="text" placeholder="<?php echo $LANG["identity"]["emailchange"]["change_email_new"]; ?>" class="textOrpasswordT" name="newemail"<?php
                    if(isset($_POST["newemail"])) {
                        	   echo ' value="'. $_POST["newemail"] .'"';
                        	} ?> />
                    <p><?php echo $LANG["identity"]["emailchange"]["change_password"]; ?>:</p>
                    <input type="password" placeholder="<?php echo $LANG["identity"]["emailchange"]["change_password"]; ?>" class="textOrpasswordT" name="password" />
                    <br />
                    ...
                    ?><button class="new-button" type="submit" name="changeEmail"><b><?php echo $LANG["global"]["send"]; ?></b><i></i></a><?php } ?>
